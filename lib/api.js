...
API.prototype.setBuildStatus = function(build, context, status, cb) {
  ...
  var self = this;
  var gitlabURL = '/projects/' + build.request.repo_id + '/statuses/' + build.request.sha + '?status=' + status.state + '&description=' + encodeURIComponent(status.description) + '&context=' + context;
  this._http( gitlabURL, 'post' )
    .send()
    .end(function(err, res) {
      if (err) {
        self.log.error({err: err, buildId: build.id, gitlabURL: gitlabURL}, '*Gitlab* An error occurred updating build status');
        return cb && cb(err);
      }

      var updatedStatus = res.status;
      self.log.info({status: updatedStatus, gitlabURL: gitlabURL}, '*Gitlab* Build status updated for build', build.id);
